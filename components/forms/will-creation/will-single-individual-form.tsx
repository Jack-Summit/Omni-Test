...
export const WillSingleIndividualForm: React.FC<WillFormProps> = ({ matter, client, onFormSubmit, onFormSave }) => {
    const [activeTab, setActiveTab] = useState("Personal/Family Information");

    const testatorLabel = client?.name || "Testator";
    ...
    const methods = useForm({
        defaultValues: {
            ...
            will_single_residual_option: "EquallyToChildren", 
    ...
    const onSubmitHandler = (data: any) => {
        if (onFormSubmit) {
            ...
            console.log("Will Questionnaire (Single Individual) Form Submitted Data (standalone):", JSON.stringify(data, null, 2));
            toast({
                title: "Form Data Captured (Standalone)",
                description: "Will information for single individual has been processed. Check console for data.",
                variant: "default",
                duration: 7000,
            });
        }
    ...
    const handleSaveDraft = () => {
        ...
        console.log("Saving Will Questionnaire (Single Individual) Draft:", JSON.stringify(currentData, null, 2));
        ...
        <FormProvider {...methods}>
            <div className="bg-card p-4 md:p-6 rounded-lg shadow-xl border border-border">
                <h2 className="text-2xl font-bold text-center text-primary mb-2">
                    Will Questionnaire (Single Individual)
                </h2>
                <p className="text-sm text-muted-foreground text-center mb-6">
                    Matter: {matter?.name || 'N/A'} | Testator: {testatorLabel}
                </p>
                ...
                <Tabs value={activeTab} onValueChange={(newTab) => { setActiveTab(newTab); window.scrollTo(0, 0); }} className="w-full">
                    ...
                    <form onSubmit={handleSubmit(onSubmitHandler)}>
                        <div className="mt-1 min-h-[400px] outline-none ring-offset-0 focus-visible:ring-0 focus-visible:ring-offset-0">
                            ...
                            {activeTab === "General Will Provisions & Distribution" && <GeneralWillProvisionsAndDistributionTab person1Label={testatorLabel} isSingleWill={true} />}
                            {activeTab === "Capacity Confirmation" && <CapacityConfirmationTab person1Label={testatorLabel} />}
                        </div>
