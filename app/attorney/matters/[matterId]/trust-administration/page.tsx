...
import { Save, PlusCircle, Trash2, Edit3, LayoutGrid, ChevronLeft, UserCheck, Edit2 as EditIcon, Search, Mail, Phone } from 'lucide-react'; 
...
import type { Matter, Contact, ContactCategory, MatterFormData, FirmUserRole, User as FirmUserType, EmailEntry, PhoneEntry } from '@/lib/types';
...
import { TrustAdminOverviewWidget } from '@/components/dashboard/matter/widgets/trust-admin/trust-admin-overview-widget';
import { TrustAdminFundingWidget } from '@/components/dashboard/matter/widgets/trust-admin/trust-admin-funding-widget';
import { TrustAdminUtcRequirementsWidget } from '@/components/dashboard/matter/widgets/trust-admin/trust-admin-utc-requirements-widget';
...
interface TrustAdminDashboardWidgetItem {
  id: string;
...
const ALL_AVAILABLE_TRUST_ADMIN_WIDGETS: Omit<TrustAdminDashboardWidgetItem, 'defaultLayout'> & { defaultLayout: Layout }[] = [
  {
    id: 'trustAdminOverview',
    component: TrustAdminOverviewWidget,
    name: 'Trust Admin Overview',
    defaultLayout: { i: 'trustAdminOverview', w: 1, h: 2, x: 0, y: 0, minW: 1, minH: 2, isDraggable: true, isResizable: true }
  },
  {
    id: 'trustAdminUtc',
    component: TrustAdminUtcRequirementsWidget,
    name: 'UTC & State Requirements',
    defaultLayout: { i: 'trustAdminUtc', w: 1, h: 2, x: 1, y: 0, minW: 1, minH: 2, isDraggable: true, isResizable: true }
  },
  {
    id: 'trustAdminFunding',
    component: TrustAdminFundingWidget,
    name: 'Asset Funding & Transfer',
    defaultLayout: { i: 'trustAdminFunding', w: 1, h: 2, x: 0, y: 2, minW: 1, minH: 2, isDraggable: true, isResizable: true }
  },
  {
    id: 'financialOverview',
    component: FinancialOverviewWidget,
    name: 'Billing for Matter',
    defaultLayout: { i: 'financialOverview', w: 1, h: 4, x: 1, y: 2, minW: 1, minH: 2, isDraggable: true, isResizable: true }
  },
  {
    id: 'pendingTasks',
    component: MatterPendingTasksWidget,
    name: 'Pending Tasks',
    defaultLayout: { i: 'pendingTasks', w: 1, h: 3, x: 0, y: 4, minW: 1, minH: 2, isDraggable: true, isResizable: true }
  },
  {
    id: 'matterDocuments',
    component: MatterDocumentsWidget,
    name: 'Matter Documents',
    defaultLayout: { i: 'matterDocuments', w: 1, h: 3, x: 1, y: 5, minW: 1, minH: 2, isDraggable: true, isResizable: true }
  },
  {
    id: 'caseNotes',
    component: MatterNotesWidget,
    name: 'Admin Log & Notes',
    defaultLayout: { i: 'caseNotes', w: 2, h: 2, x: 0, y: 7, minW: 1, minH: 2, isDraggable: true, isResizable: true }
  },
  {
    id: 'linkedContacts',
    component: MatterLinkedContactsWidget,
    name: 'Linked Contacts',
    defaultLayout: { i: 'linkedContacts', w: 2, h: 2, x: 0, y: 9, minW: 1, minH: 2, isDraggable: true, isResizable: true }
  },
];

const INITIAL_VISIBLE_TRUST_ADMIN_WIDGET_IDS = [
    'trustAdminOverview', 'trustAdminUtc', 'trustAdminFunding', 'financialOverview', 'pendingTasks', 'matterDocuments', 'caseNotes', 'linkedContacts'
];
...
const generateInitialLayoutForBreakpoint = (widgetIds: string[], numCols: number): Layout[] => {
  ...
  widgetIds.forEach((id) => {
    const widgetConfig = ALL_AVAILABLE_TRUST_ADMIN_WIDGETS.find(w => w.id === id);
...
export default function TrustAdministrationDashboardPage() {
  const { user } = useAuth();
  ...
  const [visibleWidgetIds, setVisibleWidgetIds] = useState<string[]>(INITIAL_VISIBLE_TRUST_ADMIN_WIDGET_IDS);
  const [layouts, setLayouts] = useState<Layouts>(() => generateLayoutsForAllBreakpoints(INITIAL_VISIBLE_TRUST_ADMIN_WIDGET_IDS));
  ...
  useEffect(() => {
    ...
    if (matterId && user?.firmId) {
      ...
      if (foundMatter) {
        ...
        setMatter(null);
        toast({ title: "Error", description: "Trust Administration matter not found or access denied.", variant: "destructive" });
        router.push('/attorney/matters');
      }
    }

    const storageKey = `trustAdminDashboardLayouts_${matterId}`;
    const visibleWidgetsKey = `trustAdminDashboardVisibleWidgets_${matterId}`;
    const savedLayouts = localStorage.getItem(storageKey);
    const savedVisibleWidgets = localStorage.getItem(visibleWidgetsKey);

    let currentVisibleIds = INITIAL_VISIBLE_TRUST_ADMIN_WIDGET_IDS;
    if(savedVisibleWidgets) {
        try {
            currentVisibleIds = JSON.parse(savedVisibleWidgets);
        } catch (error) {
            console.error("Failed to parse saved trust admin dashboard state, using default:", error);
        }
    }
    ...
    if (savedLayouts) {
      try {
        ...
        console.error("Failed to parse saved trust admin dashboard layouts:", error);
        setLayouts(initialLayouts);
      }
    } else {
      setLayouts(initialLayouts);
  ...
  const handleToggleEditMode = () => {
    if (isEditMode) {
      localStorage.setItem(`trustAdminDashboardLayouts_${matterId}`, JSON.stringify(layouts));
      localStorage.setItem(`trustAdminDashboardVisibleWidgets_${matterId}`, JSON.stringify(visibleWidgetIds));
      toast({title: "Layout Saved", description: "Trust Administration dashboard layout has been saved."});
    } else {
  ...
  const handleAddWidget = (widgetIdToAdd: string) => {
    ...
    const widgetConfig = ALL_AVAILABLE_TRUST_ADMIN_WIDGETS.find(w => w.id === widgetIdToAdd);
    if (!widgetConfig) return;
  ...
  const handleRemoveWidget = (widgetIdToRemove: string) => {
    const widgetConfig = ALL_AVAILABLE_TRUST_ADMIN_WIDGETS.find(w => w.id === widgetIdToRemove);
  ...
  const visibleWidgetsToRender = ALL_AVAILABLE_TRUST_ADMIN_WIDGETS.filter(w => visibleWidgetIds.includes(w.id));

  if (!isClientSide || !matter) {
    return (
      <div className="space-y-6 p-6 bg-slate-50 dark:bg-slate-900 min-h-full -m-6">
          <p>Loading trust administration dashboard...</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 bg-slate-50 dark:bg-slate-900 min-h-full -m-6 p-6">
            ...
            <div className="flex flex-1 flex-col md:flex-row md:gap-x-8 w-full">
              <div className="flex-grow mb-4 md:mb-0">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  ID: {matter.id} | Type: <span className="font-semibold text-primary">{matter.type}</span> | Status:
            ...
            {isEditMode && (
                <Dialog open={isAddWidgetModalOpen} onOpenChange={setIsAddWidgetModalOpen}>
                    ...
                    <DialogContent>
                        ...
                        {ALL_AVAILABLE_TRUST_ADMIN_WIDGETS.map(widget => (
                            <div key={widget.id} className="flex items-center space-x-2">
                            <Checkbox
                                id={`add-${widget.id}-trustadmin`}
                            ...
                            />
                            <Label htmlFor={`add-${widget.id}-trustadmin`} className="font-medium text-sm">
      ...
      {Object.keys(layouts || {}).length > 0 && visibleWidgetsToRender.length > 0 ? (
        <ResponsiveGridLayout
          ...
          draggableHandle=".drag-handle-trustadmin"
        >
            ...
            return (
              ...
              >
                {(isEditMode && (layoutItem.isDraggable ?? true) && !layoutItem.static) &&
                  <div className="drag-handle-trustadmin p-1 bg-primary cursor-move text-center text-xs text-primary-foreground opacity-80 hover:opacity-100 transition-opacity">Drag</div>
                }
