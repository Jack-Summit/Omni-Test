...
import {
  Shield, LogOut, UserCircle, FileText, CalendarDays, DollarSign, Settings, Menu, ChevronDown, LayoutDashboard
} from 'lucide-react';
...
export default function ClientLayout({
  ...
  return (
    <div className="flex-1 flex flex-col bg-gradient-to-br from-sky-600 via-sky-700 to-gray-800 text-white selection:bg-sky-200 selection:text-sky-900">
        ...
        <div className="flex items-center space-x-2 sm:space-x-4">
            ...
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="flex items-center space-x-2 focus-visible:ring-0 focus-visible:ring-offset-0 p-1 sm:p-2 rounded-full sm:rounded-md text-white hover:bg-white/20">
                <Avatar className="h-8 w-8 sm:h-9 sm:w-9">
                  <AvatarImage src={user?.avatarUrl || `https://picsum.photos/seed/${user?.email}/40/40`} alt={user?.name || "User"} data-ai-hint="professional avatar" />
          ...
          <div className="md:hidden">
            <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
              ...
              <SheetContent side="left" className="w-[250px] p-0 bg-primary/10 backdrop-blur-lg border-primary/20 text-white flex flex-col">
                <SheetHeader className="p-4 border-b border-white/20">
                  <SheetTitle className="text-lg font-semibold text-white">Navigation Menu</SheetTitle>
                </SheetHeader>
      ...
      <div className="flex flex-1 flex-col md:flex-row min-h-0"> {/* Added flex-1 and min-h-0 */}
        <NavLinks />
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8"> {/* Added overflow-y-auto */}
          {children}
        </main>
      </div>

      <footer className="text-center p-6 text-sm text-gray-300 border-t border-white/20 mt-auto">
        {currentYear && `© ${currentYear} Omni Client Portal. All information is confidential and securely managed.`}
        {!currentYear && `© Omni Client Portal. All information is confidential and securely managed.`}
      </footer>
    </div>
  );
}
